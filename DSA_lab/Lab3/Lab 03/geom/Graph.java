...
public class Graph extends GeomObject{
	...
	public void setFaceColor(Color color) {
		super.setFaceColor(color);
		for(Point2D p : points)
			p.setFaceColor(color);
	}
	...
	public static Graph sin(double a, double xMin, double xMax, double step){
		...
		for(int idx = 0; idx < N; idx++){
			double y = a*Math.sin(2*Math.PI*(1.0/(xMax - xMin))*(x - xMin));
	...
	public static Graph linear(double a, double b, double xMin, double xMax, double step) {
		int N = (int)((xMax - xMin)/step) + 1;
		Point2D[] points = new Point2D[N];
		double x = xMin;
		for(int idx = 0; idx < N; idx++){
